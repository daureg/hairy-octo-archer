if (/^u/.test(typeof define)) {
    (function(def) {
        var require = this["require"] = function(name) {
            return def[name];
        };
        this["define"] = function(name, f) {
            def[name] = def[name] || f(require);
        };
    })({});
}
...
define("minified", function() {
    ...
    var idSequence = 1;
    var DOMREADY_HANDLER = /^[ic]/.test(_document["readyState"]) ? _null : [];
    var ANIMATION_HANDLERS = {};
    var ANIMATION_HANDLER_COUNT = 0;
    ...
    function eachObj(obj, cb) {
        for (var n in obj) if (obj.hasOwnProperty(n)) cb.call(obj, n, obj[n]);
        return obj;
    }
    function each(list, cb) {
        if (list) for (var i = 0; i < list.length; i++) cb.call(list, list[i], i);
        return list;
    }
    function filterObj(obj, f) {
        var r = {};
        eachObj(obj, function(key, value) {
            if (f.call(obj, key, value)) r[key] = value;
        });
        return r;
    }
    function filter(list, filterFuncOrObject) {
        ...
        each(list, function(value, index) {
            if (f.call(list, value, index)) r.push(value);
        });
        return r;
    }
    function collector(iterator, obj, collectFunc) {
        ...
        iterator(obj, function(a, b) {
            if (isList(a = collectFunc.call(obj, a, b))) each(a, function(rr) {
                result.push(rr);
            }); else if (a != _null) result.push(a);
        });
        return result;
    }
    ...
    function values(obj, keys) {
        var list = [];
        if (keys) each(keys, function(value) {
            list.push(obj[value]);
        }); else eachObj(obj, function(key, value) {
            list.push(value);
        });
        return list;
    }
    ...
    function startsWith(base, start) {
        ...
        } else return start != _null && base.substr(0, start.length) == start;
    }
    function endsWith(base, end) {
        if (isList(base)) {
            var e2 = _(end);
            return _(base).sub(-e2.length).equals(e2) || !e2.length;
        } else return end != _null && base.substr(base.length - end.length) == end;
    }
    function reverse(list) {
        var len = list.length;
        if (isList(list)) return new M(map(list, function() {
            return list[--len];
        })); else return replace(list, /[\s\S]/g, function() {
    ...
    function copyObj(from, to, dontOverwrite) {
        eachObj(from, function(name, value) {
            if (to[name] == _null || !dontOverwrite) to[name] = value;
        });
        return to;
    }
    function extend(target) {
        for (var i = 1; i < arguments.length; i++) eachObj(arguments[i], function(name, value) {
            if (value != undef) target[name] = value;
        });
        return target;
    }
    function getFindFunc(findFunc) {
        return isFunction(findFunc) ? findFunc : function(obj, index) {
            if (findFunc === obj) return index;
        };
    }
    ...
    function find(list, findFunc, startIndex, endIndex) {
        ...
        var e = getFindIndex(list, endIndex, list.length);
        var r;
        for (var i = getFindIndex(list, startIndex, 0); i < e; i++) if ((r = f.call(list, list[i], i)) != _null) return r;
    }
    function findLast(list, findFunc, startIndex, endIndex) {
        var f = getFindFunc(findFunc);
        var e = getFindIndex(list, endIndex, -1);
        var r;
        for (var i = getFindIndex(list, startIndex, list.length - 1); i > e; i--) if ((r = f.call(list, list[i], i)) != _null) return r;
    }
    function array(list) {
    ...
    function uniq(list) {
        ...
        return filter(list, function(item) {
            if (found[item]) return _false; else return found[item] = 1;
        });
    }
    ...
    function contains(list, value) {
        for (var i = 0; i < list.length; i++) if (list[i] == value) return _true;
        return _false;
    }
    function equals(x, y) {
        ...
        var aKeys;
        if (a == b) return _true; else if (a == _null || b == _null) return _false; else if (isValue(a) || isValue(b)) return isDate(a) && isDate(b) && +a == +b; else if (isList(a)) {
            return a.length == b.length && !find(a, function(val, index) {
                if (!equals(val, b[index])) return _true;
            });
        } else {
            return !isList(b) && (aKeys = keys(a)).length == keyCount(b) && !find(aKeys, function(key) {
                if (!equals(a[key], b[key])) return _true;
            });
        }
    }
    function once(f) {
        var called = 0;
        return function() {
            if (!called++) return call(f, this, arguments);
        };
    }
    function call(f, fThisOrArgs, args) {
        if (isFunction(f)) return f.apply(args && fThisOrArgs, map(args || fThisOrArgs, nonOp));
    }
    ...
    function pad(digits, number) {
        ...
        while (preDecimal.length < digits) preDecimal = "0" + preDecimal;
        ...
        if (isDate(value)) {
            ...
            return replace(format, /(\w)(\1*)(?:\[([^\]]+)\])?/g, function(s, placeholderChar, placeholderDigits, params) {
                ...
                if (val) {
                    ...
                    if (isList(val[1])) d = (optionArray || val[1])[d]; else d = val[1](d, optionArray, timezone);
                    if (d != _null && !isString(d)) d = pad(placeholderDigits.length + 1, d);
                    return d;
                } else return s;
            });
        } else return find(format.split(/\s*\|\s*/), function(fmtPart) {
            ...
            if (match = /^([<>]?)(=?)([^:]*?)\s*:\s*(.*)$/.exec(fmtPart)) {
                ...
                if (match[1]) {
                    if (!match[2] && cmpVal1 == cmpVal2 || match[1] == "<" && cmpVal1 > cmpVal2 || match[1] == ">" && cmpVal1 < cmpVal2) return _null;
                } else if (cmpVal1 != cmpVal2) return _null;
                numFmtOrResult = match[4];
            } else numFmtOrResult = fmtPart;
            if (isNumber(value)) return numFmtOrResult.replace(/[0#](.*[0#])?/, function(numFmt) {
    ...
    function parseDate(format, date) {
        ...
        var parser = new RegExp(format.replace(/(.)(\1*)(?:\[([^\]]*)\])?/g, function(wholeMatch, placeholderChar, placeholderDigits, param) {
            if (/[dmhkyhs]/i.test(placeholderChar)) {
                ...
                return "([a-zA-Z�῿]+)";
            } else if (/w/i.test(placeholderChar)) return "[a-zA-Z�῿]+"; else if (/\s/.test(placeholderChar)) return "\\s+"; else return escapeRegExp(wholeMatch);
        }));
        if (!(match = parser.exec(date))) return undef;
        ...
        for (var i = 1; i < reIndex; i++) {
            ...
            if (isList(indexEntry)) {
                ...
                var listValue = find(valList, function(v, index) {
                    if (startsWith(matchVal.toLowerCase(), v.toLowerCase())) return index;
                });
                if (listValue == _null) return undef;
                if (placeholderChar == "a") ctorArgs[ctorIndex] += listValue * 12; else ctorArgs[ctorIndex] = listValue;
            } else if (indexEntry) {
                var value = parseInt(matchVal);
                var mapEntry = PARSE_DATE_MAP[indexEntry];
                if (isList(mapEntry)) ctorArgs[mapEntry[0]] += value - mapEntry[1]; else ctorArgs[mapEntry] += value;
            }
        }
    ...
    function parseNumber(format, value) {
        if (arguments.length == 1) return parseNumber(_null, format);
        if (/^\?/.test(format)) {
            if (!trim(value)) return _null;
            format = format.substr(1);
        }
    ...
    function dateAdd(date, property, value) {
        if (value == _null) return dateAdd(now(), date, property);
    ...
    function dateDiff(property, date1, date2) {
        ...
        var d2t = +date2;
        var dt = d2t - d1t;
        if (dt < 0) return -dateDiff(property, date2, date1);
        var propValues = {
            milliseconds: 1,
            seconds: 1e3,
            minutes: 6e4,
        ...
        for (var i = minimumResult; i < minimumResult * 1.2 + 4; i++) {
            if (+dateAddInline(d, cProp, 1) > d2t) return i;
        }
    }
    function ucode(a) {
    ...
    function template(template, escapeFunction) {
        if (templateCache[template]) return templateCache[template]; else {
            ...
            var t = function(obj, thisContext) {
                ...
                f.call(thisContext || obj, obj, function(obj, func) {
                    if (isList(obj)) each(obj, function(value, index) {
            ...
            if (templates.push(t) > MAX_CACHED_TEMPLATES) delete templateCache[templates.shift()];
    ...
    function isFunction(f) {
        return isType(f, "function") && !f["item"];
    }
    function isList(v) {
    ...
    function wordRegExpTester(name, prop) {
        ...
        return function(obj) {
            return name ? re.test(obj[prop]) : _true;
        };
    }
    ...
    function removeFromArray(array, value) {
        for (var i = 0; array && i < array.length; i++) if (array[i] === value) array["splice"](i--, 1);
    }
    ...
    function on(subSelector, eventSpec, handler, args, bubbleSelector) {
        if (isFunction(eventSpec)) return this["on"](_null, subSelector, eventSpec, handler, args); else if (isString(args)) return this["on"](subSelector, eventSpec, handler, _null, args); else return this["each"](function(baseElement, index) {
                ...
                flexiEach(toString(eventSpec).split(/\s/), function(namePrefixed) {
                    ...
                    var miniHandler = function(event, triggerOriginalTarget) {
                        ...
                        if (bubbleSelector) {
                            ...
                            while (el && el != registeredOn && !(match = selectorFilter(el))) el = el["parentNode"];
                        }
                    ...
                    registeredOn["M"][triggerId] = function(eventName, eventObj, element) {
    ...
    function ready(handler) {
        if (DOMREADY_HANDLER) DOMREADY_HANDLER.push(handler); else setTimeout(handler, 0);
    }
    function $$(selector) {
    ...
    function clone(listOrNode) {
        return collector(flexiEach, listOrNode, function(e) {
            var c;
            if (isString(e)) return e; else if (isList(e)) return clone(e); else if (isNode(e)) {
                c = e["cloneNode"](_true);
    ...
    function dollarRaw(selector, context, childOnly) {
        ...
        function filterElements(list) {
            return filter(collector(flexiEach, list, flatten), function(node) {
                var a = node;
                while (a = a["parentNode"]) if (a == context[0] || childOnly) return a == context[0];
            });
        }
        if (context) {
            if ((context = dollarRaw(context)).length != 1) return collectUniqNodes(context, function(ci) {
                return dollarRaw(selector, ci, childOnly);
            }); else if (isString(selector)) return childOnly ? filterElements(context[0].querySelectorAll(selector)) : context[0].querySelectorAll(selector); else return filterElements(selector);
        } else if (isString(selector)) return _document.querySelectorAll(selector); else return collector(flexiEach, selector, flatten);
    }
    function getFilterFunc(selector, context) {
        var nodeSet = {};
        var dotPos = nodeSet;
        if (isFunction(selector)) return selector; else if (isNumber(selector)) return function(v, index) {
        ...
        } else if (context) return function(v) {
    ...
    function getInverseFilterFunc(selector) {
        ...
        return function(v) {
            return f(v) ? _null : _true;
        };
    }
    function flexiEach(list, cb) {
        if (isList(list)) each(list, cb); else if (list != _null) cb(list, 0);
        return list;
    }
    function promise() {
        var state;
        ...
        each(assimilatedPromises, function assimilate(promise, index) {
            try {
                promise["then"](function resolvePromise(v) {
                    var then;
                    if ((isObject(v) || isFunction(v)) && isFunction(then = v["then"])) assimilate(then, index); else {
                        values[index] = map(arguments, nonOp);
                        if (++numCompleted == assimilatedNum) set(_true, assimilatedNum < 2 ? values[index] : values);
                    }
        ...
        var then = set["then"] = function(onFulfilled, onRejected) {
                            ...
                            try {
                                ...
                                if ((isObject(x) || isFunction(x)) && isFunction(then = x["then"])) {
                                    ...
                                    then["call"](x, function(x) {
                                        ...
                                        if (!cbCalled++) promise2(_false, [ value ]);
                                    });
                                    promise2["stop0"] = x["stop"];
                                } else promise2(_true, [ x ]);
                            } catch (e) {
                                if (!cbCalled++) promise2(_false, [ e ]);
                            }
                        })(call(f, undef, values));
            ...
            if (state != _null) setTimeout(callCallbacks, 0); else deferred.push(callCallbacks);
            return promise2;
        };
        set["always"] = function(func) {
            return then(func, func);
        };
        set["error"] = function(func) {
            return then(0, func);
        };
        return set;
    }
    function M(list, assimilateSublists) {
        ...
        if (list) for (var i = 0, len = list.length; i < len; i++) {
            var item = list[i];
            if (assimilateSublists && isList(item)) for (var j = 0, len2 = item.length; j < len2; j++) self[idx++] = item[j]; else self[idx++] = item;
        } else self[idx++] = assimilateSublists;
        self["length"] = idx;
        self["_"] = _true;
    }
    function _() {
    ...
    copyObj({
        ...
        text: function() {
            return collector(flexiEach, this, function(e) {
        ...
        trav: function(property, selector, maxDepth) {
            ...
            return new M(collectUniqNodes(this, function(node) {
                var r = [];
                var c = node;
                while ((c = c[property]) && r.length != max) if (f(c)) r.push(c);
                return r;
            }));
        },
        ...
        get: function(spec, toNumber) {
            ...
            if (element) {
                if (isString(spec)) {
                    ...
                    var s;
                    if (spec == "$") s = element.className; else if (spec == "$$") {
                        s = element.getAttribute("style");
                    } else if (spec == "$$fade" || spec == "$$show") {
                        if (self["get"]("$visibility") == "hidden" || self["get"]("$display") == "none") s = 0; else if (spec == "$$fade") {
                            s = isNaN(self["get"]("$opacity", _true)) ? 1 : self["get"]("$opacity", _true);
                        } else s = 1;
                    } else if (spec == "$$slide") s = self["get"]("$height"); else if (match[1] == "$") {
                        s = _window["getComputedStyle"](element, _null)["getPropertyValue"](replace(match[2], /[A-Z]/g, function(match2) {
                            return "-" + match2.toLowerCase();
                        }));
                    } else if (match[1] == "@") s = element.getAttribute(match[2]); else s = element[match[2]];
        ...
        set: function(name, value) {
            ...
            if (value !== undef) {
                ...
                if (name == "$$fade") {
                ...
                } else flexiEach(self, function(obj, c) {
                    ...
                    if (name == "$") {
                        flexiEach(newValue && newValue.split(/\s+/), function(clzz) {
                            ...
                            if (/^\+/.test(clzz) || cName == clzz && oldClassName == className) className += " " + cName;
                        ...
                        if (newValue != _null) obj.setAttribute(match[2], newValue); else obj.removeAttribute(match[2]);
                    } else if (match[1] == "$") obj["style"][match[2]] = newValue; else obj[match[2]] = newValue;
                });
            } else if (isString(name) || isFunction(name)) this["set"]("$", name); else eachObj(name, function(n, v) {
                self["set"](n, v);
            });
            return self;
        },
        ...
        add: function(children, addFunction) {
            return this["each"](function(e, index) {
                ...
                (function appendChildren(c) {
                    if (isList(c)) flexiEach(c, appendChildren); else if (isFunction(c)) appendChildren(c(e, index)); else if (c != _null) {
                        var n = isNode(c) ? c : _document.createTextNode(c);
                        if (lastAdded) lastAdded["parentNode"]["insertBefore"](n, lastAdded["nextSibling"]); else if (addFunction) addFunction(n, e, e["parentNode"]); else e.appendChild(n);
                        lastAdded = n;
                    }
        ...
        addBefore: function(children) {
            return this["add"](children, function(newNode, refNode, parent) {
        ...
        addFront: function(children) {
            return this["add"](children, function(newNode, refNode) {
        ...
        animate: function(properties, duration, linearity) {
            ...
            var dials = collector(flexiEach, this, function(li, index) {
                ...
                eachObj(dialStartProps = elList.get(properties), function(name, start) {
                    ...
                    dialEndProps[name] = isFunction(dest) ? dest(start, index, li) : name == "$$slide" ? getNaturalHeight(elList, dest) : dest;
                });
        ...
        dial: function(properties1, properties2, linf) {
            ...
            function getColorComponent(colorCode, index) {
                return /^#/.test(colorCode) ? parseInt(colorCode.length > 6 ? colorCode.substr(1 + index * 2, 2) : (colorCode = colorCode.charAt(1 + index)) + colorCode, 16) : parseInt(replace(colorCode, /[^\d,]+/g).split(",")[index]);
            }
            return function(t) {
                eachObj(properties1, function(name, start) {
                    var end = properties2[name], i = 0;
                    self["set"](name, t <= 0 ? start : t >= 1 ? end : /^#|rgb\(/.test(end) ? "rgb(" + Math.round(interpolate(getColorComponent(start, i), getColorComponent(end, i++), t)) + "," + Math.round(interpolate(getColorComponent(start, i), getColorComponent(end, i++), t)) + "," + Math.round(interpolate(getColorComponent(start, i), getColorComponent(end, i++), t)) + ")" : replace(end, /-?[\d.]+/, toString(interpolate(extractNumber(start), extractNumber(end), t))));
                });
            };
        },
            ...
            if (stateDesc2) {
                ...
                return function(newState) {
                    if (newState !== state) {
                        ...
                        if (durationMs) (promise = self["animate"](stateDesc, promise ? promise["stop"]() : durationMs, linearity))["then"](function() {
                            promise = _null;
                        }); else self["set"](stateDesc);
                    }
                };
            } else return self["toggle"](replace(stateDesc1, /\b(?=\w)/g, "-"), replace(stateDesc1, /\b(?=\w)/g, "+"));
        },
        values: function(data) {
            var r = data || {};
            this["each"](function(el) {
                var n = el["name"], v = toString(el["value"]);
                if (/form/i.test(el["tagName"])) for (var i = 0; i < el["elements"].length; i++) $(el["elements"][i])["values"](r); else if (n && (!/kbox|dio/i.test(el["type"]) || el["checked"])) {
        ...
        onOver: function(subSelect, toggle) {
            ...
            if (!toggle) return this["onOver"](_null, subSelect); else return this["on"](subSelect, "|mouseover |mouseout", function(ev, index) {
        ...
        onFocus: function(selector, handler) {
            if (!handler) return this["onFocus"](_null, selector); else return this["on"](selector, "|focus", handler, [ _true ])["on"](selector, "|blur", handler, [ _false ]);
        },
        onChange: function onChange(subSelect, handler) {
            var oldValues = [];
            if (handler) return this["each"](function(el, index) {
                function register(eventNames, property) {
                    $(el)["on"](subSelect, eventNames, function() {
                        handler.call(this, el[property], index);
                    });
                }
            ...
            }); else return this["onChange"](_null, subSelect);
        },
        ...
        per: function(subSelector, handler) {
            if (isFunction(subSelector)) for (var len = this.length, i = 0; i < len; i++) subSelector.call(this, new M(_null, this[i]), i); else $(subSelector, this)["per"](handler);
            return this;
        },
        ht: function(htmlTemplate, object) {
            return this["set"]("innerHTML", isFunction(htmlTemplate) ? htmlTemplate(object) : /{{/.test(htmlTemplate) ? formatHtml(htmlTemplate, object) : /^#\S+$/.test(htmlTemplate) ? formatHtml($$(htmlTemplate)["text"], object) : htmlTemplate);
        }
    }, M.prototype);
    copyObj({
        request: function(method, url, data, settings0) {
            ...
            try {
                ...
                if (dataIsMap && /post/i.test(method)) xhr["setRequestHeader"]("Content-Type", "application/x-www-form-urlencoded");
                ...
                xhr["onreadystatechange"] = function() {
                    if (xhr["readyState"] == 4 && !callbackCalled++) {
                        if (xhr["status"] == 200) prom(_true, [ xhr["responseText"], xhr["responseXML"] ]); else prom(_false, [ xhr["status"], xhr["statusText"], xhr["responseText"] ]);
                    }
                };
                xhr["send"](data);
            } catch (e) {
                if (!callbackCalled) prom(_false, [ 0, _null, toString(e) ]);
            }
            return prom;
        },
        ...
        loop: function(paintCallback) {
            ...
            var currentTime = 0;
            var id = idSequence++;
            var requestAnim = _window["requestAnimationFrame"] || function(f) {
                setTimeout(function() {
                    f(+new Date());
                }, 33);
            };
            function raFunc(ts) {
                eachObj(ANIMATION_HANDLERS, function(id, f) {
                    f(ts);
                });
                if (ANIMATION_HANDLER_COUNT) requestAnim(raFunc);
            }
            function stop() {
        ...
        setCookie: function(name, value, dateOrDays, dontEscape) {
            _document.cookie = name + "=" + (dontEscape ? value : escape(value)) + (dateOrDays ? "; expires=" + (isObject(dateOrDays) ? dateOrDays : new Date(+new Date() + dateOrDays * 864e5)).toUTCString() : "");
        },
    ...
    copyObj({
        ...
        range: function(start, end) {
            ...
            for (var i = end != _null ? start : 0; i < e; i++) r.push(i);
            return new M(r);
        },
        bind: bind,
        partial: partial,
